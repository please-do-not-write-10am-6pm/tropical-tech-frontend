			...
			<View style={styles.container}>
				...
				<View style={{ marginTop: 40 }}>
					<View style={[styles.rowContainer, { justifyContent: "space-between" }]}>
						<Text>{`${dataHotel[code].value.toFixed(2)}$ X 1 adult`}</Text>
						<Text>{`${dataHotel[code].value.toFixed(2)}$`}</Text>
					</View>
					<View
						style={[
							styles.rowContainer,
				...
				<Text style={[styles.title, { marginTop: 15 }]}>Things to know</Text>
				<Text>Guest requirements</Text>
				<Text>Up to 10 guests, ages 18 and up can attend</Text>
				...
				>
					<View
						style={[styles.rowContainer, { width: "30%", alignItems: "center" }]}
					>
						{userData && userData?.creditCard.length > 0 ? (
							...
							<>
								...
								>{`Add card`}</Text>
								{addNewCard && (
									<Text style={styles.requiredCard}>{`Required for purchase`}</Text>
								)}
							</>
						)}
					</View>
					...
					>
						<Text style={{ textDecorationLine: "underline", fontFamily: "Corbel" }}>
							Edit
						</Text>
					</TouchableOpacity>
				</View>
				...
				<Text style={{ fontFamily: "Corbel", fontSize: 16 }}>
					Any experience can be cancelled and fully refunded within 24 hours of
				...
				<Text style={{ marginTop: 22, fontFamily: "Corbel" }}>
					By selecting the button below, you agree to the{" "}
					<Text
						style={{ textDecorationLine: "underline", fontFamily: "Corbel-Bold" }}
					>
						Guest Release and Waiver, the Cancellation Policy, the Guest Refund Policy
						and social-distancing and other COVID-19-related guidelines.
					</Text>{" "}
						...
						if (
							...
							if (purchase?.length === 0 || purchase?.length === undefined) {
								...
								setCashback([
									{
										day: new Date().toUTCString(),
										inValue: authStatus?.isAuthenticated
											? (dataHotel[code].value * 10) / 100
											: 0,
										transactionName: `Reservation-${dataHotel[code].name}`,
										type: "IN",
